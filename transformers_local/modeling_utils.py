    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if state_dict is None and not from_tf:
            ...
            for k, v in state_dict.items():
                if "encoder" in k:
                    additional_state_dict.update({k.replace("encoder", "question_encoder"): v, k.replace("encoder", "words_encoder"): v})
                if "decoder" in k:
                    additional_state_dict.update({k.replace("decoder", "cls_decoder"): v, k.replace("decoder", "generate_decoder"): v})
                    if "encoder_attn" in k:
                        additional_state_dict.update(
                            {k.replace("decoder", "generate_decoder").replace("encoder_attn", "question_encoder_attn"): v, k.replace("decoder", "generate_decoder").replace("encoder_attn", "words_encoder_attn"): v})
                        # , k.replace("decoder", "generate_decoder").replace("encoder_attn", "cls_decoder_attn"): v

            state_dict.update(additional_state_dict)
            # except Exception:
            #     raise OSError(
